...
#check dircetory
if [ -d "/data/projects/fate/common" || -d "/data/projects/fate/eggroll" || -d "/data/projects/fate/fateboard" || -d "/data/projects/fate/python" || -d "/data/projects/fate/example" ]
then
  echo "Warning: please rename /data/projects/fate/{common|eggroll|fateboard|python|example}"
fi

if [ -d "/data/projects/data/fate/mysql" ]
then
  echo "Warning: please rename /data/projects/data/fate/mysql"
fi

num=$( ls /data/projects/common/supervisord/supervisord.d/fate-*.conf | wc -l )
if [ $num -gt 0 ]
then
  echo "Warning: supervisor_fate_conf exists, please remove /data/projects/common/supervisord/supervisord.d/fate-*.conf and update all"
fi
