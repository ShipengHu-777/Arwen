...
from federatedml.param.boosting_param import HeteroPipeBoostParam, DecisionTreeParam
from federatedml.ensemble.basic_algorithms import HeteroPipeDecisionTreeGuest
from federatedml.util import consts
from federatedml.transfer_variable.transfer_class.hetero_pipe_boosting_predict_transfer_variable import \
    HeteroPipeBoostTransferVariable
...
class HeteroPipeBoostingTreeGuest(HeteroBoostingGuest):

    def __init__(self):
        super(HeteroPipeBoostingTreeGuest, self).__init__()
        ...
        self.model_param = HeteroPipeBoostParam()
        self.complete_secure = False
        self.data_alignment_map = {}
        self.predict_transfer_inst = HeteroPipeBoostTransferVariable()
        self.model_name = 'HeteroPipeBoost'
    ...
    def _init_model(self, param: HeteroPipeBoostParam):

        super(HeteroPipeBoostingTreeGuest, self)._init_model(param)
    ...
    def fit_a_booster(self, epoch_idx: int, booster_dim: int):
        ...
        tree = HeteroPipeDecisionTreeGuest(tree_param=self.tree_param)
    ...
    def load_booster(self, model_meta, model_param, epoch_idx, booster_idx):
        tree = HeteroPipeDecisionTreeGuest(self.tree_param)
    ...
    @staticmethod
    def traverse_a_tree(tree: HeteroPipeDecisionTreeGuest, sample, cur_node_idx):
    ...
    @staticmethod
    def traverse_trees(node_pos, sample, trees: List[HeteroPipeDecisionTreeGuest]):
        ...
        for t_idx, tree in enumerate(trees):
            ...
            rs, reach_leaf = HeteroPipeBoostingTreeGuest.traverse_a_tree(tree, sample, cur_node_idx)
    ...
    @staticmethod
    def add_y_hat(leaf_pos, init_score, learning_rate, trees: List[HeteroPipeDecisionTreeGuest], multi_class_num=None):
    ...
    @staticmethod
    def get_predict_scores(leaf_pos, learning_rate, init_score, trees: List[HeteroPipeDecisionTreeGuest]
                           , multi_class_num=-1, predict_cache=None):

        if predict_cache:
            init_score = 0  # prevent init_score re-add

        predict_func = functools.partial(HeteroPipeBoostingTreeGuest.add_y_hat,
    ...
    def boosting_fast_predict(self, data_inst, trees: List[HeteroPipeDecisionTreeGuest], predict_cache=None,
                              pred_leaf=False):
