...
class Boosting(ModelBase, ABC):
    ...
    def data_format_transform(row):
        ...
        if type(row.features).__name__ != consts.SPARSE_VECTOR:
            ...
            new_row.features.set_ndarray(np.array(indices), np.array(data))
            # '''
            '''
            row.features[np.isnan(row.features)] = 0
            row.features[abs(row.features) < consts.FLOAT_ZERO] = 0
            sparse_dict = {i:row.features[i] for i in np.nonzero(row.features)[0]}
            new_row = copy.deepcopy(row)
            new_row.features = SparseVector(dict_=sparse_dict, shape=feature_shape)
            '''
            ...
            '''
            for key, value in sparse_vec.items():
                if value == NoneType() or np.isnan(value):
                    replace_key.append(key)
