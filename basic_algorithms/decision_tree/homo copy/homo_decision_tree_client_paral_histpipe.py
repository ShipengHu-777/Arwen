#homo_decision_tree_client_paral_histpipe.py
import functools
...
from federatedml.ensemble import FeatureHistogramWeights
...
class HomoDecisionTreeClient(DecisionTree):
    ...
    def get_left_node_local_histogram(self, cur_nodes: List[Node], tree: List[Node], g_h, table_with_assign,
        ...
        LOGGER.info("start to get node histograms")
        #there is a parameter '0' after parameter g_h, its type is int, and it means adopting no Optimization in feature_histogram.py
        histograms = self.hist_computer.calculate_histogram(
            table_with_assign, g_h,0,
            split_points, sparse_point,
            valid_feature, node_map,
            self.use_missing, self.zero_as_missing)
    ...
    Fit & Predict
    """
    def send_hist_pipeline(self,dep,epoch_idx,tree_idx,local_histogram):
        for j in range(4):
            # para:0 means batch_id
            recv_histogram_suffix=(0, dep, epoch_idx, tree_idx, "32", j)
            send_histogram_suffix=(0, dep, epoch_idx, tree_idx, "21", j)
            signal_suffix=(0, dep, self.epoch_idx, self.tree_idx, "23", j+1)
            node_remote_histogram = self.transfer_inst.node_histogram.get(idx=3, suffix=recv_histogram_suffix)
            LOGGER.debug('histpipe {}:server2: remote histogram received at layer {}'.format(j,dep))
            if j == 3:
                node_local_histogram = FeatureHistogramWeights(list_of_histogram_bags=local_histogram[j*len(local_histogram)//4:])
            else:
                self.transfer_inst.signal.remote(0, role=consts.HOST, idx=1, suffix=signal_suffix)
                node_local_histogram = FeatureHistogramWeights(list_of_histogram_bags=local_histogram[j*len(local_histogram)//4:(j+1)*len(local_histogram)//4])
            '''
            for histbag in node_local_histogram.hists:
                bag = histbag.bag
                for component_idx in range(len(bag)):
                    #new_list = []
                    for hist_idx in range(len(bag[component_idx])):  
                        bag[component_idx][hist_idx][0] = float(bag[component_idx][hist_idx][0])
                        bag[component_idx][hist_idx][1] = float(bag[component_idx][hist_idx][1])
                        bag[component_idx][hist_idx][2] = float(bag[component_idx][hist_idx][2])
            '''
            node_sum_histogram = functools.reduce(model_add, [node_remote_histogram, node_local_histogram])
            self.transfer_inst.node_histogram.remote(node_sum_histogram, role=consts.HOST, idx=0, suffix=send_histogram_suffix)
            LOGGER.debug('histpipe {}:server2: sum histogram sent at layer {}'.format(j,dep))
    ...
    def fit(self):
        ...
        self.transfer_inst.root_info.remote(dict_weights, role=consts.ARBITER, idx=0, suffix=('root_node_sync1', self.epoch_idx))
        g_h_dict = self.transfer_inst.root_info.get(idx=0, suffix=('root_node_sync2', self.epoch_idx))._weights
        global_g_sum, global_h_sum = g_h_dict['g_sum'], g_h_dict['h_sum']
        ...
        for dep in range(tree_height):
            ...
            if dep<4:
                ...
                for batch_id, i in enumerate(range(0, len(self.cur_layer_node), self.max_split_nodes)):
                    ...
                    local_node_histogram_suffix=(batch_id, dep, self.epoch_idx, self.tree_idx)
                    weights = FeatureHistogramWeights(list_of_histogram_bags=local_histogram)
                    for histbag in weights.hists:
                        bag = histbag.bag
                        for component_idx in range(len(bag)):
                            #new_list = []
                            for hist_idx in range(len(bag[component_idx])):
                                ...
                                bag[component_idx][hist_idx][1] = float(bag[component_idx][hist_idx][1])
                                bag[component_idx][hist_idx][2] = float(bag[component_idx][hist_idx][2])
                    self.transfer_inst.node_histogram.remote(weights, role=consts.ARBITER, idx=0, suffix=local_node_histogram_suffix)
                    LOGGER.debug('local histogram sent at layer {}'.format(dep))
                ...
                if dep == 4:
                    LOGGER.debug('getting left_cur_layer_node')
                    self.left_cur_layer_node=self.cur_layer_node[0:len(self.cur_layer_node)//2]
                    self.right_cur_layer_node=self.cur_layer_node[len(self.cur_layer_node)//2:]
                    LOGGER.debug('getting left_inst2node_idx')
                    self.left_inst2node_idx=self.inst2node_idx.filter(lambda k,v: v[1]<=22)
                    self.right_inst2node_idx=self.inst2node_idx.filter(lambda k,v: v[1]>22)
                    LOGGER.debug('getting left_data_bin')
                    self.left_data_bin=self.data_bin.join(self.left_inst2node_idx, lambda v1,v2:v1)
                    self.right_data_bin=self.data_bin.join(self.right_inst2node_idx, lambda v1,v2:v1)
                    LOGGER.debug('getting left_g_h')
                    self.left_g_h=self.g_h.join(self.left_inst2node_idx,lambda v1,v2:v1)
                    self.right_g_h=self.g_h.join(self.right_inst2node_idx,lambda v1,v2:v1)
                    LOGGER.debug('getting left_table_with_assignment')
                    left_table_with_assignment = self.left_data_bin.join(self.left_inst2node_idx, lambda inst, assignment: (inst, assignment))
                    right_table_with_assignment = self.right_data_bin.join(self.right_inst2node_idx, lambda inst, assignment: (inst, assignment))
                    LOGGER.debug('getting left_table_with_assignment done')

                if dep + 1 == tree_height:
                    #receiving right tree best split points
                    t.join()
                    split_info = self.sync_best_splits(suffix=((dep-1)*2+1, self.epoch_idx))
                    LOGGER.debug('got right tree best splits from arbiter')
                    #wait until all the clients get their right tree best splits results
                    self.sync_cur_layer_node_num(len(self.cur_layer_node), suffix=((dep-1)*2+1+100, self.epoch_idx, self.tree_idx))
                    split_info_useless = self.sync_best_splits(suffix=((dep-1)*2+1+100, self.epoch_idx))
                    LOGGER.debug('Finish sync:got right tree best splits from arbiter')
                    #update right tree
                    new_layer_node = self.update_tree(self.right_cur_layer_node, split_info)
                    self.right_cur_layer_node = new_layer_node
                    self.right_inst2node_idx, leaf_val = self.assign_instances_to_new_node(right_table_with_assignment, self.tree_node)
                    # record leaf val
                    if self.sample_weights is None:
                        self.sample_weights = leaf_val
                    else:
                        self.sample_weights = self.sample_weights.union(leaf_val)
                    LOGGER.debug('assigning instance to right tree new nodes done')

                    #self.cur_layer_node=self.left_cur_layer_node.append(self.right_cur_layer_node)
                    self.inst2node_idx=self.left_inst2node_idx.union(self.right_inst2node_idx,lambda v1,v2:v1+v2)

                    for node in self.left_cur_layer_node:
                        node.is_leaf = True
                        self.tree_node.append(node)
                    for node in self.right_cur_layer_node:
                        node.is_leaf = True
                        self.tree_node.append(node)
                ...
                #start computing left tree histograms
                if dep>4:
                    left_table_with_assignment = self.left_data_bin.join(self.left_inst2node_idx, lambda inst, assignment: (inst, assignment))
                split_info, agg_histograms = [], []
                for batch_id, i in enumerate(range(0, len(self.left_cur_layer_node), self.max_split_nodes)):
                    cur_to_split = self.left_cur_layer_node[i:i+self.max_split_nodes]

                    node_map = self.get_node_map(nodes=cur_to_split)
                    LOGGER.debug('left tree node map is {}'.format(node_map))
                    LOGGER.debug('computing left tree histogram for batch{} at depth{}'.format(batch_id, dep))
                    local_histogram = self.get_left_node_local_histogram(
                        cur_nodes=cur_to_split,
                        tree=self.tree_node,
                        g_h=self.left_g_h,
                        table_with_assign=left_table_with_assignment,
                        split_points=self.bin_split_points,
                        sparse_point=self.bin_sparse_points,
                        valid_feature=self.valid_features
                    )
                ...
                if dep>4:
                    ...
                    #send left tree best split points
                    LOGGER.debug('federated finding left tree best splits for batch{} at layer {}'.format(batch_id, dep-1))
                    #self.sync_local_node_histogram(local_histogram, suffix=(batch_id, dep*2, self.epoch_idx, self.tree_idx))
                    local_hist_para=local_histogram
                    local_histogram=[]
                    t=threading.Thread(target=self.send_hist_pipeline,args=(dep*2,self.epoch_idx, self.tree_idx,local_hist_para,))
                    t.start()
                ...
                if dep==4:
                    #send left tree best split points
                    LOGGER.debug('federated finding left tree best splits for batch{} at layer {}'.format(batch_id, dep))
                    #self.sync_local_node_histogram(local_histogram, suffix=(batch_id, dep*2, self.epoch_idx, self.tree_idx))
                    local_hist_para=local_histogram
                    local_histogram=[]
                    t=threading.Thread(target=self.send_hist_pipeline,args=(dep*2,self.epoch_idx, self.tree_idx,local_hist_para,))
                ...
                #start computing right tree histograms
                if dep>4:
                    right_table_with_assignment = self.right_data_bin.join(self.right_inst2node_idx, lambda inst, assignment: (inst, assignment))
                split_info, agg_histograms = [], []
                for batch_id, i in enumerate(range(0, len(self.right_cur_layer_node), self.max_split_nodes)):
                    cur_to_split = self.right_cur_layer_node[i:i+self.max_split_nodes]

                    node_map = self.get_node_map(nodes=cur_to_split)
                    LOGGER.debug('right tree node map is {}'.format(node_map))
                    LOGGER.debug('computing right tree histogram for batch{} at depth{}'.format(batch_id, dep))
                    local_histogram = self.get_left_node_local_histogram(
                        cur_nodes=cur_to_split,
                        tree=self.tree_node,
                        g_h=self.right_g_h,
                        table_with_assign=right_table_with_assignment,
                        split_points=self.bin_split_points,
                        sparse_point=self.bin_sparse_points,
                        valid_feature=self.valid_features
                    )
                ...
                #receiving left tree best split points
                t.join()
                split_info = self.sync_best_splits(suffix=(dep*2, self.epoch_idx))
                LOGGER.debug('got left tree best splits from arbiter')
                #wait until all the clients get their left tree best splits results
                self.sync_cur_layer_node_num(len(self.cur_layer_node), suffix=(dep*2+100, self.epoch_idx, self.tree_idx))
                split_info_useless = self.sync_best_splits(suffix=(dep*2+100, self.epoch_idx))
                LOGGER.debug('Finish sync:got left tree best splits from arbiter')

                #send right tree best split points
                LOGGER.debug('federated finding right tree best splits for batch{} at layer {}'.format(batch_id, dep))
                #self.sync_local_node_histogram(local_histogram, suffix=(batch_id, dep*2+1, self.epoch_idx, self.tree_idx))
                local_hist_para=local_histogram
                local_histogram=[]
                t=threading.Thread(target=self.send_hist_pipeline,args=(dep*2+1,self.epoch_idx, self.tree_idx,local_hist_para,))
                t.start()

                #update left tree
                new_layer_node = self.update_tree(self.left_cur_layer_node, split_info)
                self.left_cur_layer_node = new_layer_node
                self.left_inst2node_idx, leaf_val = self.assign_instances_to_new_node(left_table_with_assignment, self.tree_node)
                # record leaf val
                if self.sample_weights is None:
                    self.sample_weights = leaf_val
                else:
                    self.sample_weights = self.sample_weights.union(leaf_val)
                LOGGER.debug('assigning instance to left tree new nodes done')
