...
class FeatureHistogram(object):
    ...
    def compute_histogram(self, dep, data_bin, grad_and_hess, unique_val_table,bin_split_points, bin_sparse_points,
        ...
        if ret == 'tensor':
            ...
            histograms = self.calculate_histogram(data_bin, grad_and_hess,unique_val_table,
            ...
            if not sparse_optimization:
                ...
                histogram_table = self.calculate_histogram(data_bin, grad_and_hess,unique_val_table,
                                                           bin_split_points, bin_sparse_points,
                                                           valid_features, to_compute_node_map,
    ...
    def calculate_histogram(self, data_bin, grad_and_hess,unique_val_table,
                            bin_split_points, bin_sparse_points,
                            valid_features=None,
                            node_map=None,
                            use_missing=False,
        ...
        if batch_histogram_intermediate_rs.count() == 0: # if input sample number is 0, return empty histograms
            ...
            batch_histogram_cal = functools.partial(
                FeatureHistogram._batch_calculate_histogram,unique_val_table=unique_val_table,
        ...
        for k in range(node_num):
            ...
            for fid in range(bin_split_points.shape[0]):
                ...
                if valid_features is not None and valid_features[fid] is False:
                    ...
                    #LOGGER.debug("bin_split_points[fid]shape is {}".format(bin_split_points[fid].shape[0]))
    ...
    @staticmethod
    def _batch_calculate_histogram(kv_iterator, unique_val_table,bin_split_points=None,
        ...
        if homo_sbt_optim:
            for data_id, value in kv_iterator:
                ...
                if (g,h) not in unique_val_table:
                    unique_val_table[(g,h)]=[data_record]
                else:
                    unique_val_table[(g,h)].append(data_record)
                data_record += 1
                if len(unique_val_table)>520:
                    homo_sbt_optim=False
                    break
        ...
        if homo_sbt_optim:
            ...
            LOGGER.info(f'traverse kv_iter and gen unique_val_table {time.time() - time_start}')
            '''
            LOGGER.info(f'running optim hist compute')
            node_num = len(node_map)
            ...
            node_histograms = FeatureHistogram._generate_histogram_template(node_map, bin_split_points, valid_features,
                                                                            missing_bin)
            # 下面32是分箱数目， 2000是featrue数目
            node_histograms_keycount=[[[[0 for i in range(32)] for j in range(2000)] for m in range(node_num)] for n in range(len(unique_val_table))]
            LOGGER.debug('len unique_val_table is {}'.format(len(unique_val_table)))
            count=0
            time_a=time.time()
            '''
            LOGGER.info(f'init time {time.time() - time_start}')

            for gh_key in unique_val_table:
                for rid in unique_val_table[gh_key]:
                    node_idx = node_map.get(node_ids[rid])
                    for fid, value in data_bins[rid].features.get_all_data():
                        node_histograms_keycount[count][node_idx][fid][value] = node_histograms_keycount[count][node_idx][fid][value]
                        node_histograms_keycount[count][node_idx][fid][value] = node_histograms_keycount[count][node_idx][fid][value]

            LOGGER.info(f'only traverse {time.time() - time_start}')
            '''

            for gh_key in unique_val_table:
                #delta4cur_gh=1<<(count*16)
                for rid in unique_val_table[gh_key]:
                    # node index is the position in the histogram list of a certain node
                    node_idx = node_map.get(node_ids[rid])
                    # node total sum value
                    zero_opt_node_sum[node_idx][0] += gh_key[0]
                    zero_opt_node_sum[node_idx][1] += gh_key[1]
                    ...
                    for fid, value in data_bins[rid].features.get_all_data():
                        ...
                        if use_missing and value == NoneType():
                            ...
                            value = -1
                        #node_histograms_keycount[node_idx][fid][value] += delta4cur_gh
                        node_histograms_keycount[count][node_idx][fid][value] += 1
                count+=1
            LOGGER.info(f'analyse repeat key {time.time() - time_start}')
            for node_id in range(len(node_histograms)):
                for fid in range(len(node_histograms[node_id])):
                    total_g=0
                    total_h=0
                    total_num=0
                    for value in range(len(node_histograms[node_id][fid])):
                        g_sum=0
                        h_sum=0
                        num_sum=0
                        count=0
                        for key in unique_val_table:
                            #raw_num=node_histograms_keycount[node_id][fid][value]
                            #num=(raw_num >> (16*count)) & 65535
                            num=node_histograms_keycount[count][node_id][fid][value]                          
                            g_sum+=key[0]*num
                            h_sum+=key[1]*num
                            num_sum+=num
                            count+=1
                        node_histograms[node_id][fid][value]=[g_sum,h_sum,num_sum]
                        total_g+=g_sum
                        total_h+=h_sum
                        total_num+=num_sum
                    # node feature total sum value
                    zero_optim[node_id][fid]=[total_g,total_h,total_num]
            # LOGGER.debug("optim calculating histogram done, time elapsed {} s".format(time.time()-time_a))
            LOGGER.info(f'compute end {time.time() - time_start}')
            ...
            LOGGER.info(f'final complete {time.time() - time_start}')
            ...
            LOGGER.info(f'running non-optim hist compute')
            data_bins = []
            node_ids = []
            grad = []
            hess = []
            ...
            LOGGER.debug("normal calculating histogram done, time elapsed {} s".format(time.time()-time_c))
