...
class HomoDecisionTreeClient(DecisionTree):
    ...
    def fit(self):
        ...
        for dep in range(tree_height):
            ...
            LOGGER.debug('oneoneone start to fit layer {}'.format(dep))
            aa=time.time()
            ...
            for batch_id, i in enumerate(range(0, len(self.cur_layer_node), self.max_split_nodes)):
                ...
                LOGGER.debug('twotwotwo federated finding best splits for batch{} at layer {}'.format(batch_id, dep))
                bb=time.time()
            ...
            LOGGER.debug('threethreethree got best splits from arbiter')
            cc=time.time()
            ...
            LOGGER.debug('fourfourfour assigning instance to new nodes done')
            dd=time.time()
            LOGGER.debug('time detail on dep:{}, total time is:{}, sync time is:{}'.format(dep,dd-aa,cc-bb))
            LOGGER.info(f'{dep} time {time.time() - start}')
