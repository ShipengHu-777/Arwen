...
class HomoDecisionTreeClient(DecisionTree):
    ...
    def fit(self):
        ...
        elif consts.TransArch == "Hetero":
            # Heterogeneous
            g_h_dict = self.transfer_inst.root_info.get(idx=3, suffix=('root_node_sync1_32', self.epoch_idx))._weights
            g_sum += g_h_dict['g_sum']; h_sum += g_h_dict['h_sum']
            d = {'g_sum': g_sum, 'h_sum': h_sum}
            dict_weights = DictWeights(d=d)
            self.transfer_inst.root_info.remote(dict_weights, role=consts.HOST, idx=0, suffix=('root_node_sync1_21', self.epoch_idx))
            sum_g_h_dict = self.transfer_inst.root_info.get(idx=3, suffix=('root_node_sync2_32', self.epoch_idx))
            self.transfer_inst.root_info.remote(sum_g_h_dict, role=consts.HOST, idx=0, suffix=('root_node_sync2_21', self.epoch_idx))
            global_g_sum, global_h_sum = (sum_g_h_dict._weights)['g_sum'], (sum_g_h_dict._weights)['h_sum']
        ...
        for dep in range(tree_height):
            ...
            for batch_id, i in enumerate(range(0, len(self.cur_layer_node), self.max_split_nodes)):
                ...
                if consts.TransArch == "Homo" or len(local_histogram)<8:
                    # Homogeneous
                    ...
                    # Heterogeneous
                    if consts.HistBagPipe == False:
                        recv_histogram_suffix=(batch_id, dep, self.epoch_idx, self.tree_idx, "32")
                        send_histogram_suffix=(batch_id, dep, self.epoch_idx, self.tree_idx, "21")
                        node_remote_histogram = self.transfer_inst.node_histogram.get(idx=3, suffix=recv_histogram_suffix)
                        node_local_histogram = FeatureHistogramWeights(list_of_histogram_bags=local_histogram)
                        ...
                        LOGGER.debug('local histogram sent at layer {}'.format(dep))
                    elif consts.HistBagPipe == True:
                        '''
                        for idx, hist_bag in enumerate(local_histogram):
                            recv_histogram_suffix=(batch_id, dep, self.epoch_idx, self.tree_idx, "32", idx)
                            send_histogram_suffix=(batch_id, dep, self.epoch_idx, self.tree_idx, "21", idx)
                            signal_suffix=(batch_id, dep, self.epoch_idx, self.tree_idx, "23", idx)

                            node_remote_histogram = self.transfer_inst.node_histogram.get(idx=3, suffix=recv_histogram_suffix)

                            node_local_histogram = FeatureHistogramWeights(list_of_histogram_bags=[hist_bag])
                            node_sum_histogram = functools.reduce(model_add, [node_remote_histogram, node_local_histogram])
                        ...
                        # '''
                        for j in range(4):
                            recv_histogram_suffix=(batch_id, dep, self.epoch_idx, self.tree_idx, "32", j)
                            send_histogram_suffix=(batch_id, dep, self.epoch_idx, self.tree_idx, "21", j)

                            # '''
                            signal_suffix=(batch_id, dep, self.epoch_idx, self.tree_idx, "23", j+1)
                            node_remote_histogram = self.transfer_inst.node_histogram.get(idx=3, suffix=recv_histogram_suffix)
                            ...
                            signal_suffix=(batch_id, dep, self.epoch_idx, self.tree_idx, "01", j)
                            self.transfer_inst.node_histogram.remote(node_local_histogram, role=consts.HOST, idx=0, suffix=send_histogram_suffix)
                            LOGGER.debug('histpipe {}:server2: sum histogram sent at layer {}'.format(j,dep))
                            self.transfer_inst.signal.get(idx=2, suffix=signal_suffix)
                            '''
