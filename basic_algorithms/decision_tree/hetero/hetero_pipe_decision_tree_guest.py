...
from federatedml.transfer_variable.transfer_class.hetero_pipe_decision_tree_transfer_variable import \
    HeteroPipeDecisionTreeTransferVariable
...
class HeteroPipeDecisionTreeGuest(DecisionTree):

    def __init__(self, tree_param):
        super(HeteroPipeDecisionTreeGuest, self).__init__(tree_param)

        self.encrypter = None
        self.encrypted_mode_calculator = None
        self.transfer_inst = HeteroPipeDecisionTreeTransferVariable()
    ...
    def init(self, flowid, runtime_idx, data_bin, bin_split_points, bin_sparse_points, valid_features,
        ...
        super(HeteroPipeDecisionTreeGuest, self).init_data_and_variable(flowid, runtime_idx, data_bin, bin_split_points,
        ...
        self.cur_data_bin = self.data_bin
    ...
    def update_instances_node_positions(self):
        self.cur_data_bin = self.cur_data_bin.join(self.inst2node_idx, lambda data_inst, _: data_inst)
        ###
        self.grad_and_hess = self.grad_and_hess.join(self.inst2node_idx, lambda g_h, _: g_h)
        ###
        self.cur_data_bin = self.cur_data_bin.pipe_reorder(self.inst2node_idx)
        self.grad_and_hess = self.grad_and_hess.pipe_reorder(self.inst2node_idx)
        self.inst2node_idx = self.inst2node_idx.pipe_reorder(self.inst2node_idx)
        self.data_with_node_assignments = self.cur_data_bin.join(self.inst2node_idx, lambda data_inst, dispatch_info: (
    ...
    def fit(self):
        ...
        # LOGGER.debug('self.data_bin = {}'.format(list(self.data_bin.collect())))
        # LOGGER.debug('self.grad_and_hess = {}'.format(list(self.grad_and_hess.collect())))
        LOGGER.debug('self.inst2node_idx = {}'.format(list(self.inst2node_idx.collect())))
        ...
        for dep in range(self.max_depth):
            ...
            LOGGER.debug('dep: {}, self.inst2node_idx = {}'.format(dep, list(self.inst2node_idx.collect())))
        LOGGER.info("finish train_loop")
